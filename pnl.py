...
while 1:
    ...
    for p in positionsJSON:
        ...
        if percPNL <=-5:
            print('\033[1m',p['market'],'\033[0m', ' | ', int(p['notionalSize']), ' | ', round(p['markPrice'],3), ' | ', round(p['averageOpenPrice'],3), ' | ', round(p['breakEvenPrice'],3), ' | ', round(p['estimatedLiquidationPrice'],3), ' | ', '\x1b[0;41;1m', round(p['pnl'],2),' | ',round(percPNL,1),'%','\x1b[0m') 
        elif percPNL > -5 and percPNL < 0: 
